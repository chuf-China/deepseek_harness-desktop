...
rem release.cmd ??? git add -A ????????????????????? WIP/???????????????????????????????????? commit???
rem ?????????????????????????????????????????????????????????????????????????????? commit/stash ???????????????
rem ?????????????????? %TEMP%??????????????????????????? git status ???????????????????????????????????????
git -C "%PROJ%" status --porcelain > "%TEMP%\dsh-release-dirty.tmp" 2>nul
set "DIRTY="
set /p DIRTY=<"%TEMP%\dsh-release-dirty.tmp"
del "%TEMP%\dsh-release-dirty.tmp" 2>nul
if defined DIRTY (
